...
class CustomFutureWishListBuilderEmpty<T> extends StatelessWidget {
  ...
  const CustomFutureWishListBuilderEmpty({
    Key? key,
        ...
        if (snapshot.connectionState == ConnectionState.waiting) {
          ...
          if (snapshot.hasError || snapshot.data == null) {
            ...
            if (snapshot.data == [] || GetUtils.isBlank(snapshot.data)!) {
              return reloadWidget ??
                  NoContent(
                     img: Assets.EMPTY_WISHLIST,
                    title: errorTitle ?? "Your Wishlist is Empty !!",
